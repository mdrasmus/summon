...
\begin{tabular}{ll}
    ...
    q                          & close window \\
    \`\  + left click          & display pop-up menu (note: back-tic is
                                 usually above tab key) \\
\end{tabular}
\vspace{.2in}
...
\code{summon/examples/summatrix} and \code{summon/examples/sumtree} directories.
Both programs will print their usage if run with no arguments.  View/execute the
\code{view\_*.sh} scripts for examples of how to call \code{summatrix} and 
\code{sumtree}.
...
(\code{summatrix}) or  instantied as an object
(\code{summon.matrix.MatrixViewer}) from within your own python program. 
SUMMATRIX also includes support for visualizating data clustering and other
matrix permutations.  To see all of SUMMATRIX's options run \code{summatrix} on
the commmand-line with no options.
...
\code{--rlab=$filename$}, \code{--clab=$filename$} command-line options.  If
...
on an entry in the matrix.  Labels can also be displayed the sides of the
...
\code{http://glaros.dtc.umn.edu/gkhome/views/cluto}.
